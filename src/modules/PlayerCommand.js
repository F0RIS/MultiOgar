...
PlayerCommand.list = {
    ...
    ophelp: function(args) {
        ...
        if (isNaN(page) || (page != 1 && page != 2))
            return this.writeLine("INFO: Type /help 1 to display the first page of OP keys (there are 2 total).");
        if (page == 1) {
    ...
    addbot: function(args) {
        ...
        if (isNaN(add))
            return this.writeLine("ERROR: Invalid amount of bots to add!");
    ...
    board: function(args) {
        ...
        var newLB = [];
        var input = args[1];
        if (args.length > this.gameServer.config.serverMaxLB + 1)
            return this.writeLine("ERROR: The limit for lines of text on the leaderboard is " + this.gameServer.config.serverMaxLB + "!");
    ...
    c/*hange*/: function(args) {
        if (!this.playerTracker.OP.enabled)
            return this.writeLine("WARN: You must have OP mode to use this command!");
        if (args.length < 3)
            return this.writeLine("ERROR: Please specify a valid value for this config!");
        var key = args[1];
        var value = args[2];
        if (value.indexOf('.') != -1) value = parseFloat(value);
        else value = parseInt(value);
        if (value == null || isNaN(value))
            return this.writeLine("ERROR: Invalid value: " + value + "!");
        if (!this.gameServer.config.hasOwnProperty(key))
            return this.writeLine("ERROR: Unknown config value: " + key + "!");
        ...
        Log.info(this.playerTracker._name + " changed the config value '" + key + "' to " + this.gameServer.config[key] + ".");
    },
    chat: function (args) {
        if (!this.playerTracker.OP.enabled)
            return this.writeLine("WARN: You must have OP mode to use this command!");
        for (var i = 0; i < this.gameServer.clients.length; i++) {
            this.gameServer.sendChatMSG(null, i, String(args.slice(1, args.length).join(" ")));
        }
    ...
    color: function(args) {
        ...
        if (isNaN(color.r) || isNaN(color.g) || isNaN(color.b))
            return this.writeLine("ERROR: Please specify a valid RGB color!");
        client.color = color;
    ...
    explode: function() {
        ...
        for (var i in client.cells) {
            for (var cell = client.cells[i]; cell._size > 31.63;) {
                ...
                if (config.ejectMaxSize > loss)
                    loss = Math.random() * (config.ejectMaxSize - loss) + loss;
    ...
    kickbots: function(args) {
        ...
        var removed = 0;
        for (var i = 0; i < this.gameServer.clients.length; i++) {
            if (this.gameServer.clients[i].connected != null) continue;
    ...
    kill: function(args) {
        ...
        if (!this.playerTracker.cells.length)
            return this.writeLine("ERROR: You're not spawned in the game!");
    ...
    lms: function() {
        ...
        this.gameServer.disableSpawn ?
            this.writeLine("Last Man Standing has been enabled.") :
            this.writeLine("Last Man Standing has been disabled.");
    },
    mass: function(args) {
        if (!this.playerTracker.OP.enabled)
            return this.writeLine("WARN: You must have OP mode to use this command!");
        ...
        if (!this.playerTracker.cells.length)
            return this.writeLine("ERROR: You are either dead or not playing!");
    ...
    merge: function(args) {
        ...
        if (client.mergeOverride) this.writeLine("You are now merging.");
        else this.writeLine("You are not merging anymore.");
    },
    minion: function(args) {
        ...
        if (isNaN(add) && add != "remove")
            return this.writeLine("ERROR: Invalid number of minions to add!");
    ...
    pause: function() {
        ...
        !this.gameServer.running ? this.writeLine("You paused the game.") : this.writeLine("You un-paused the game.");
    },
    rec: function() {
        if (!this.playerTracker.OP.enabled)
            return this.writeLine("WARN: You must have OP mode to use this command!");
        this.playerTracker.recMode = !this.playerTracker.recMode;
        this.playerTracker.recMode ? this.writeLine("You now have rec mode.") : this.writeLine("You no longer have rec mode.");
    },
    reload: function() {
        if (!this.playerTracker.OP.enabled)
            return this.writeLine("WARN: You must have OP mode to use this command!");
    ...
    replace: function(args) {
        ...
        if (ent != "virus" && ent != "food" && ent != "mothercell")
            return this.writeLine("ERROR: Please specify either virus, food, or mothercell!");
        var client = this.playerTracker;
        if (!client.cells.length) return this.writeLine("ERROR: You are either dead or not playing!");
        ...
        if (ent == "food") this.writeLine("Replaced your cells with food cells.");
        else if (ent == "virus") this.writeLine("Replaced your cells with viruses.");
        else if (ent == "mothercell") this.writeLine("Replaced your cells with mother cells.");
    },
    reset: function(args) {
        if (!this.playerTracker.OP.enabled)
            return this.writeLine("WARN: You must have OP mode to use this command!");
    ...
    split: function(args) {
        ...
        if (client.cells.length >= this.gameServer.config.playerMaxCells)
            return this.writeLine("ERROR: You have reached the splitting limit of " + this.gameServer.config.playerMaxCells + "!");
    ...
    tp/*teleport*/: function(args) {
        if (!this.playerTracker.OP.enabled)
            return this.writeLine("WARN: You must have OP mode to use this command!");
        var pos = {x: parseInt(args[1]), y: parseInt(args[2])};
    ...
    op: function() {
        ...
        if (this.playerTracker.OP.enabled) this.writeLine("You now have OP mode.");
        else if (!this.playerTracker.OP.enabled) this.writeLine("You no longer have OP mode.");
    },

...
GameServer.prototype.socketEvent = function(ws) {
    ...
    if (this.config.serverIpLimit) {
        ...
        for (var i = 0; i < this.clients.length; i++) {
            ...
            if (!socket.connected || socket.remoteAddress != ws._socket.remoteAddress) continue;
            ipConnections++;
        }
        ...
        if ((ws.lastAliveTime - this.startTime) / 1000 >= config.minionIgnoreTime) {
            if (this.minionTest.length >= config.minionThreshold) {
                ...
                for (var i = 0; i < this.minionTest.length; i++) {
                    ...
                    if (!playerTracker.socket.connected) continue;
...
GameServer.prototype.randomColor = function() {
    ...
    if (config.serverColorType == 1) { // Ogar-Unlimited's random color system
        RGB = [0xFF, 0x07, (Math.random() * 255) >> 0];
    ...
    if (config.serverColorType == 2) { // Old Ogar's random color system
        i = Math.floor(Math.random() * this.oldColors.length);
        RGB = this.oldColors[i];
        return {
            r: RGB.r,
            g: RGB.g,
            b: RGB.b
...
GameServer.prototype.monotoneColors = function() {
    var color = Math.floor(255 * Math.random()) + 0;
    var config = this.config;
...
GameServer.prototype.onChatMSG = function(from, to, message) {
    ...
    if (!this.config.serverChat || (from && from.isMuted)) return;
    if (message.length > 64) message = message.slice(0, 64);
    if (this.config.serverChatAscii) {
        for (var i = 0; i < message.length; i++) {
            var c = message.charCodeAt(i);
            if (c < 0x20 || c > 0x7F)
                if (from) return this.sendChatMSG(null, from, "You can only use ASCII text!");
        }
    }
    if (this.checkBadWord(message) && this.config.filterBadWords)
        if (from) return this.sendChatMSG(null, from, "You cannot use bad words in the chat!");
...
GameServer.prototype.movePlayer = function(cell, client) {
    if (client.socket.connected === 0 || client.frozen) return;
...
GameServer.prototype.checkRigidCollision = function(m) {
    ...
    if (m.cell.owner != m.check.owner) {
        if (this.gameMode.haveTeams && m.check.owner.isMi || m.cell.owner.isMi && !this.config.minionTeamCollision) return 0;
        else return this.gameMode.haveTeams && m.cell.owner.team == m.check.owner.team;
    }
...
GameServer.prototype.resolveRigidCollision = function(m) {
    ...
    if (m.cell.cellType == 3 && this.config.ejectCollisionType == 1) {
        ...
        var rt = m.cell._size + m.check._size;
        var r1 = ~~m.cell._size / rt;
        var r2 = ~~m.check._size / rt;
...
GameServer.prototype.resolveCollision = function(m) {
    ...
    if (this.config.gravitationalPushsplits)
        if (!check.canEat(cell) || cell.getAge() < 1 && check.cellType == 0) return;
...
GameServer.prototype.willCollide = function(pos, s) {
    var bound = {
        minX: pos.x - s,
        minY: pos.y - s,
        maxX: pos.x + s,
    ...
    var d = bound.minX * bound.minX + bound.minY * bound.minY;
    if (d + (s * s) <= (s * 2)) return null;
...
GameServer.prototype.splitCells = function(client) {
    ...
    for (var i = 0; i < client.cells.length; i++) {
        ...
        if (cell._size > config.playerMinSplit) {
            if (!cellToSplit.push(cell), !client.recMode) var max = config.playerMaxCells;
            else max = (config.playerMaxCells * config.playerMaxCells) * 2;
            if (cellToSplit.length + client.cells.length >= max) break;
        }
    }
    for (i = 0; i < cellToSplit.length; i++) {
        var splitCells = 0;
        cell = cellToSplit[i];
        ...
        isNaN(angle) && (angle = Math.PI / 2),
        this.splitPlayerCell(client, cell, angle, null, max) && splitCells++;
    }
};
    ...
    try {
        if (fs.existsSync(config)) {
            ...
            for (var r in i) this.config.hasOwnProperty(r) ?
                this.config[r] = i[r] :
                Log.error("Unknown config.ini value: " + r);
        } else Log.warn("Config not found... Generating new config"),
        fs.writeFileSync(config, ini.stringify(this.config), "utf-8");
    } catch (ini) {
        Log.error(ini.stack), Log.error("Failed to load " + config + ": " + ini.message);
    }
...
GameServer.prototype.loadBadWords = function() {
    ...
    try {
        if (!fs.existsSync(badWordFile)) {
            Log.warn(badWordFile + " not found!");
        } else {
            ...
            this.badWords = words;
            if (!this.config.filterBadWords) Log.info("The bad word filter is disabled.");
            else Log.info(this.badWords.length + " bad words loaded.");
        }
    } catch (err) {
        Log.error(err.stack);
...
GameServer.prototype.loadUserList = function() {
    if (!this.config.serverUserRoles) {
        return Log.info("User roles are disabled.");
    }
...
WebSocket.prototype.sendPacket = function(packet) {
    ...
    if (packet == null || socket.connected == null || socket.playerTracker.isMi) return;
...
GameServer.prototype.getStats = function() {
    ...
    for (var i = 0, len = this.clients.length; i < len; i++) {
        var socket = this.clients[i];
        if (!socket || !socket.connected || socket.playerTracker.isMi) continue;
        total++;
...
GameServer.prototype.pingServerTracker = function() {
    ...
    for (var i = 0, len = this.clients.length; i < len; i++) {
        var socket = this.clients[i];
        if (!socket || socket.connected === 0) continue;
        if (socket.connected == null) bots++;
        else {
            total++;
            if (socket.playerTracker.cells.length) alive++;
            else spectate++;
...
function trackerRequest(options, type, body) {
    ...
    options.headers['user-agent'] = 'MultiOgar-Edited 1.5.1';
